...
def parallel_train_worker(rank, 
    ...
    try:
        ...
        print(f"Worker {rank} finished. Control Episode Reward: {round(ep_reward, 2)}. Design Reward (Unnormalized): {round(design_reward_unnorm, 2)}. Worker puts None in queue.")
        train_queue.put((rank, None, design_reward_unnorm))  # Signal that this worker is done 
...
def parallel_eval_worker(rank, 
    ...
    for i in range(eval_worker_config['n_iterations']):
        ...
        state, _ = env.reset(extreme_edge_dict, eval_worker_config['num_proposals'], tl = tl, real_world=real_world, eval_mode=True, eval_iteration=i)
